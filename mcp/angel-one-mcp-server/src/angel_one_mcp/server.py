...
from .rate_limiter import AsyncRequestGate, TTLCache, gated_call
...
REQUEST_GATE = AsyncRequestGate(min_interval=float(os.getenv("ANGEL_ONE_MIN_REQUEST_INTERVAL", "1.5")))
QUOTE_CACHE = TTLCache()
SYMBOL_CACHE = TTLCache()
QUOTE_CACHE_TTL = float(os.getenv("ANGEL_ONE_QUOTE_CACHE_TTL", "60"))

SUPPORTED_GAINERS_LOSERS_TYPES = {
    "PercPriceGainers", "PercPriceLosers", "PercOIGainers", "PercOILosers",
}
LEGACY_GAINERS_LOSERS_ALIASES = {
    "PercGainers": "PercPriceGainers",
    "PercLosers": "PercPriceLosers",
}


def normalize_gainers_losers_request(datatype: str, expirytype: str = "NEAR") -> dict[str, str]:
    """Normalize documented SmartAPI mover types and reject unsafe guesses."""
    normalized = LEGACY_GAINERS_LOSERS_ALIASES.get(datatype, datatype)
    if normalized not in SUPPORTED_GAINERS_LOSERS_TYPES:
        allowed = ", ".join(sorted(SUPPORTED_GAINERS_LOSERS_TYPES))
        raise ValueError(f"Unsupported datatype {datatype!r}; use one of: {allowed}")
    expiry = expirytype.upper()
    if expiry not in {"NEAR", "NEXT", "FAR"}:
        raise ValueError("expirytype must be NEAR, NEXT, or FAR")
    return {"datatype": normalized, "expirytype": expiry}

# Official SmartAPI instrument-master mappings for the broad indices. These
# are used only after validation through read-only quote calls.
MARKET_INDICES = {
    "NIFTY_50": {"exchange": "NSE", "tradingsymbol": "Nifty 50", "symboltoken": "99926000"},
...
async def get_ltp_data(exchange: str, tradingsymbol: str, symboltoken: str) -> Dict[str, Any]:
    ...
    try:
        await ensure_authenticated()
        cache_key = f"ltp:{exchange}:{tradingsymbol}:{symboltoken}"
        cached = QUOTE_CACHE.get(cache_key)
        if cached is not None:
            return cached
        ltp_data = await gated_call(REQUEST_GATE, smart_api.ltpData, exchange, tradingsymbol, symboltoken)
        QUOTE_CACHE.set(cache_key, ltp_data, QUOTE_CACHE_TTL)
        return ltp_data
    except Exception as e:
        return handle_api_error("get_ltp_data", e)

@mcp.tool()
async def get_market_data(mode: str, exchange_tokens: Dict[str, List[str]]) -> Dict[str, Any]:
    """Read-only Angel One market snapshot for exchange/token batches.

    mode must be LTP, OHLC, or FULL. exchange_tokens maps exchange names
    such as NSE/BSE/NFO to symbol-token lists. No order operation is performed.
    """
    try:
        await ensure_authenticated()
        if mode.upper() not in {"LTP", "OHLC", "FULL"}:
            return {"status": False, "error": "mode must be LTP, OHLC, or FULL"}
        cache_key = f"quote:{mode.upper()}:{exchange_tokens}"
        cached = QUOTE_CACHE.get(cache_key)
        if cached is not None:
            return cached
        result = await gated_call(REQUEST_GATE, smart_api.getMarketData, mode.upper(), exchange_tokens)
        QUOTE_CACHE.set(cache_key, result, QUOTE_CACHE_TTL)
        return result
    except Exception as e:
        return handle_api_error("get_market_data", e)
...
async def get_candle_data(
    ...
    try:
        ...
        candle_data = await gated_call(REQUEST_GATE, smart_api.getCandleData, historic_params)
        return candle_data
...
async def search_scrip(exchange: str, searchscrip: str) -> Dict[str, Any]:
    ...
    Args:
        exchange: Exchange (NSE, BSE, NFO, MCX)
        searchscrip: Company name or partial symbol. For broad indices, use the
            validated MARKET_INDICES mappings rather than guessing a token.
    """
    try:
        await ensure_authenticated()
        cache_key = f"search:{exchange}:{searchscrip.upper().strip()}"
        cached = SYMBOL_CACHE.get(cache_key)
        if cached is not None:
            return cached
        response = await gated_call(REQUEST_GATE, smart_api.searchScrip, exchange=exchange, searchscrip=searchscrip)
        SYMBOL_CACHE.set(cache_key, response, 86400)
        return response
    except Exception as e:
...
async def get_gainers_losers(datatype: str, expirytype: str = "NEAR") -> Dict[str, Any]:
    ...
    Args:
        datatype: SmartAPI type: PercPriceGainers, PercPriceLosers,
            PercOIGainers, or PercOILosers. Legacy PercGainers/PercLosers
            aliases are normalized to price movers.
        expirytype: Derivatives expiry type (NEAR, NEXT, FAR). The endpoint
            returns derivatives movers, not cash-equity gainers/losers.
    """
    try:
        await ensure_authenticated()
        
        gl_params = normalize_gainers_losers_request(datatype, expirytype)
        gainers_losers = await gated_call(REQUEST_GATE, smart_api.gainersLosers, gl_params)
...
def main():
    ...
    try:
        # Credentials are validated lazily when an authenticated tool is called.
        # This lets MCP clients discover the server before local credentials exist.
